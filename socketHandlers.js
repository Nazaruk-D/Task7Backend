...
function handleSocket(io) {
    return function (socket) {
        ...
        socket.on('join-game', (data) => {
            ...
            io.to(id1).to(id2).emit('join-game-success', {
                gameId: game.id,
                gameName: game.gameName,
        ...
        socket.on('make-move', (data) => {
            ...
            if (gameName === "bullsAndCows") {
                ...
                if (bulls === 4) {
                    playerIds.forEach((id) => {
                        io.to(id).emit('game-over', {
                            ...
                            board,
                            gameName: game.gameName,
                            userMoveId: game.userMoveId,
                            bulls,
                            cows,
                            winner: game.userMoveId
                        });
                        bullsAndCowsGames = bullsAndCowsGames.filter((g) => g.id !== gameId);
                    })
                } else {
                    playerIds.forEach((id) => {
                        io.to(id).emit('check-number-result', {
                            gameId,
                            board,
                            gameName: game.gameName,
                            userMoveId: game.userMoveId,
                ...
                if (winner) {
                    ...
                    playerIds.forEach((id) => {
                        io.to(id).emit('update-game-state', {
                            gameId,
                            board,
                            gameName: game.gameName,
                            userMoveId: game.userMoveId
